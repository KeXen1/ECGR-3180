...
public:
    SNode() : next(nullptr) {} // Constructor, which points next pointer to a nullptr
};
...
// Your code here
	while(!l1.empty()){
		...
		l1.removeFront();
	} // Goes through l1, adds the front, and remove the front to go to the next node.
	
	while(!l2.empty()){
		lout.addBack(l2.front());
		l2.removeFront();
	} // Same process of l1 for l2
}
